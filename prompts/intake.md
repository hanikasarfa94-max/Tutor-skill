...
Let me ask a few questions 鈥?you can skip any of them.
```
...
```
What should we call this mentor? (A name, abbreviation, or code 鈥?dashes for spaces)

Examples: prof-zhang, close-reading-archetype, methodology-archetype, strict-driver-archetype
```

- Accept any string
- Generate slug: lowercase, spaces 鈫?dashes, Chinese 鈫?pinyin-dashes
...
What kind of mentor is this?
  ...
  1. Specific person 鈥?you have materials from a real mentor
  2. Archetype 鈥?no single person, building from a type (e.g. "close-reading mentor", "methodology-focused mentor", "strict progress-driving mentor")
  3. Hybrid 鈥?starting from an archetype, then injecting personal materials

(Type a number or describe in your own words)
```

Map to `mentor_type`:
- `specific` 鈥?a real person with source materials
- `archetype` 鈥?a constructed type without private materials
- `hybrid` 鈥?archetype as base, personal materials overlaid
...
Extract:
- `discipline` 鈥?the broad field
- `research_fields` 鈥?list of specific areas
...
```
What materials do you have? List anything you can share 鈥?papers, emails, books,
...
Material categorization:
  ...
  Category A (Academic Method):
    - [item] 鈫?[why it's Category A]
    ...

  Category B (Mentoring Behavior):
    - [item] 鈫?[why it's Category B]
    ...

  Category C (Bridge):
    - [item] 鈫?[why it's Category C]
    ...

  鈿?Missing / thin coverage:
...
```
In your own words 鈥?how would you describe this mentor's academic style and how they work with students?
...
What do you most want this mentor skill to be good at?
  ...
  1. Review 鈥?critique outlines, drafts, and papers
  2. Diagnose 鈥?sharpen research problems and directions
  3. Advance 鈥?manage task progression and overcome stagnation
  4. All three equally
...
Mentor intake summary:
  ...
  Materials:
    Category A (Method):   {count} items 鈥?{brief list}
    Category B (Persona):  {count} items 鈥?{brief list}
    Category C (Bridge):   {count} items 鈥?{brief list}
...
1. `method_analyzer.md` 鈥?on Category A materials
2. `mentoring_style_analyzer.md` 鈥?on Category B materials
3. `bridge_analyzer.md` 鈥?on Category C materials
4. `mentor_builder.md` 鈥?integrate all outputs
...
to help build the persona. Answer in your own words 鈥?skip anything you're not sure about.

1. When a student presents a vague or ill-formed idea, what does this mentor typically do?
2. How do they deliver critical feedback 鈥?directly, through questions, indirectly?
3. Do they have a low or high tolerance for hand-waving and vagueness?
4. How do they assign tasks 鈥?specific and detailed, or open-ended?
